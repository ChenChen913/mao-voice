...
- **v5.14 阿里复审第 12~13 轮**：9 个 high 全部修复（orchestrate 工作区缺失跳过/真实路径校验、模型下载 .part 原子改名、热键队列锁、设置窗线程安全、密钥不落盘、stop 超时可重启、64 位句柄）；另修词库原子写、.env.example 白名单、词库重试缓存、CI 加固等。
