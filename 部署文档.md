...
- **迷你波形胶囊悬浮窗**：录音时只显示一个 150×52 的胶囊 + 5 根随真实音量跳动的音频波，不抢焦点。
...
IDLE →（按一下右Alt）→ RECORDING →（再按一下右Alt / 静音超时）→ PROCESSING
     ...
     → PREVIEW（悬浮窗预览结果，含草稿平滑过渡）
     → INJECTING（剪贴板 + Ctrl+V 注入）
     → IDLE
```

- 录音期间每约 2 秒对新增音频做一次**增量草稿转写**，悬浮窗实时显示（可选）。
...
| 文件 | 职责 |
| --- | --- |
| `main.py` | 入口；状态机（toggle）；录音/转写/润色/注入管线编排；线程安全轮询 |
| `hotkey.py` | 全局热键监听（pynput），单击 toggle + 200ms 防抖 + AltGr 别名 |
| `recorder.py` | 麦克风录音；RMS 电平回调；增量草稿；VAD 接入 |
...
| `ui.py` | 悬浮窗：胶囊 + 音频波动画、状态切换、不抢焦点（WS_EX_NOACTIVATE） |
...
2. **按一下右 Alt** 开始录音——屏幕底部中央出现迷你胶囊，5 根音频波随声音跳动。
